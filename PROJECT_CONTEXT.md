...
## Billing: release, invoice attach, and a portal Billing tab — the missing verbs on an already-designed noun (2026-09-12) — user-asked, real invoice supplied

**The ask**: given the real commercial workflow (Zoho generates the tax invoice, ops uploads it
monthly), how much of a "Billing" tile is buildable right now? Answered by reading the code rather
than guessing: `BillingInvoice` was already fully designed and migrated at MS-08 — its own comment,
written before this conversation, already says *"the tax invoice is generated in Zoho and uploaded
here, never generated by this app"* — but nothing had ever written to it. There was no release
action either: the billing board has `canRelease` checks and status labels for one, but
`releasedAt` was set nowhere in the codebase, which is why the portal's "Saved in rupees" tile had
never been able to show a figure.

**The three missing verbs, built on the existing schema and permission model, not a new design**:
`attachInvoice` (ops, `requireBillingOps` — the gate's own docstring already named this act),
`releaseCalculation` (the accountant only, `requireAccountant` — CON-33's "ops cannot release its
own output," untouched), `recordPayment` (ops). Sequenced as FEAT-053-AC-1/AC-3 actually specifies,
which is stricter than the first draft assumed: the Zoho invoice is attached to a *calculated* (not
yet released) month, reconciled against the app's own commercial total, and release is refused
until the reconciliation is a match or an acknowledged mismatch — so CON-13's payment clock is
always keyed to a real, human-confirmed invoice, never an assumed one. `src/lib/
invoice-reconciliation.ts` is the pure decision module (`refuseRelease`, `refuseInvoiceAttach`,
`reconcileInvoiceAmount`), 17 unit cases, following this codebase's own "factor the decision out,
keep the action a thin shell" convention.

**A found-and-fixed bug before this was ever tested**: the first draft of `releaseCalculation` set
`BillingInvoice.releasedAt` but never `MonthlyCalculation.releasedAt` — a different field the rest
of the app actually reads (`portal-energy.ts`'s `rupeesSaved`, and everything else gated on a
released month). Caught by re-reading the schema's own field list before wiring the action, not by
testing — would have shipped a Release button that silently did nothing for every ₹ figure
downstream.

**The GST reconciliation is the one genuinely new piece of arithmetic, and it is a stated
assumption**: the app's own commercial total is pre-tax; the real sample invoice
(FT/2026-27/055 — Sub Total ₹14,050, IGST18 ₹2,529, Total ₹16,579) confirms Zoho's invoice adds a
flat 18% on top. `reconcileInvoiceAmount()` compares the invoice against `computedTotal × 1.18`,
not against the raw total — a naive equality check would read every correct invoice as an 18%
"mismatch." `ASSUMED_GST_RATE = 0.18` is named and dated as an assumption, not asserted as a rule:
if a future invoice carries a different tax treatment, this is the one place to revisit.

**A new PortalGrant, `billing`** — deliberately not folded into the existing `documents` grant
(whose own note said "reports, invoices, agreement" before this): a due date and a payment status
read as an action item a member might not be trusted with, where a filed report does not carry the
same weight. `/portal/billing` lists only RELEASED invoices (an `attached` one is still
mid-reconciliation on the back office's own screen — CON-33's whole point is that a figure reaches
a society only once something other than the process that produced it says so), scoped by the
viewer's own societyId (INV-05) both in the list query and again in the download action, so an
invoice id alone is never proof of the right to read it.

**"Pay Now" is honestly not a payment gateway** — none exists anywhere in this codebase. The
portal states the invoice can be paid by bank transfer using the details printed on the PDF itself,
and that FirsThing confirms payment manually; nothing pretends to process one.

**Deliberately not built, stated rather than silently narrowed**: no correction path for a wrongly
attached invoice (a second attach is refused outright, matching this codebase's own "no edit-in-
place" rule for rescale events and documents — a real mistake needs its own void-and-reattach path
built, not invented on the spot); CON-13's automatic overdue/warning/suspended progression is not
wired to any job — `recordPayment` only ever sets `paid`, so a real defaulting society does not get
auto-suspended by this pass, which was not asked for and is a materially bigger, more consequential
piece of automation than "let ops upload the invoice they already have."

**Verified 18/19 in a browser, using the real sample invoice's own numbers throughout** (the one
failure was a test's own insufficient wait before checking rendered text — confirmed by re-running
with a longer wait and getting the exact expected sentence; every database-level assertion in the
same run passed on the first try): ops attaches FT/2026-27/055 at ₹16,579 against a ₹14,050
computed total and it reconciles as matched (the exact GST arithmetic proven, not just accepted);
ops is never offered Release; the accountant is, and releasing sets both the calculation's and the
invoice's own `releasedAt`; the resident portal shows the real amount, dates and a Pending chip;
recording a payment flips it to Paid on both the admin and portal screens. 810 unit tests (17 new),
...
**The pre-install consumption has three bases, and the sheet starts on the higher** (the user's
