...
- **Acceptance criteria:**
  ...
  - AC-6 (worksheet — amended 2026-09-15, the user's redesign): Given the offer form, when the operator prices a deal, then the inputs are the figures the negotiation actually has — per light type: lights to install as per the agreement, the pre-installation consumption of those lights on a **chosen basis** — the demo circuit's measured baseline extrapolated per light, the theoretical draw of the agreed lights (watts × hours a day, from the load inventory and editable), or a custom typed figure; a fresh sheet defaults to the higher of the first two ("we decide based on which is higher") and the basis is stored on the circuit term — and the agreed savings % (defaulting to the demo's figure and recorded as negotiated when changed); then the unit rate and the total expected saving in ₹ (bound both ways — editing either recomputes the other), the monthly amount payable to FirsThing, and spare lights as per the agreement. The society's take (saving − fee) and both parties' share percentages are derived and displayed, never typed; the stored `revenueSharePct` is the derived society share. The projected saving in kWh and ₹ is stored on the offer so it keeps saying what it was priced on (INV-02).
