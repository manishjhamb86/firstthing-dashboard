...
**Last updated:** 2026-09-22 (MS-08 closure reconciliation: scope notes on FEAT-049/059/060 stating what covers each for the invoice-first primary path and what stays deferred to phase two alongside FEAT-048; see PROJECT_CONTEXT.md). Previously 2026-09-15 (Feature-mode pass, CON-47: FEAT-109..111 added — invoice-first monthly billing and published-month portal stats; scope notes on FEAT-048/053/054/101). Before that, 2026-08-12 (gate review: 12 open items resolved; FEAT-032/FEAT-052 rewritten after CON-24/CON-25 were corrected. **Post-gate audit sweep, same day:** CON-11's per-light-type metering and per-circuit tolerance bands applied across CAP-01/02/04/05; CAP-22 notifications added (FEAT-090..093); CAP-02's no-demo variant given its own brief (FEAT-094). **Phase 4 feedback, 2026-08-12:** the flow mapping in `04-flows-system-map.md` exposed 9 missing features, added here as FEAT-095..103) · **Mode:** Ecosystem
...
- **Acceptance criteria:**
  ...
  - AC-7 (correction — amended 2026-09-16, the user's call: "on installation light count can change"): Given a recorded inventory area, when field staff correct its count (and method / note — an estimated count still needs its note), then the row is updated with old and new logged, the extrapolation-base totals re-read from it, and the candidate circuit for that light type follows the corrected total — forward only, as a RepresentedCountChange effective the current month naming the inventory (earlier months and an issued offer's own snapshot keep saying what they were computed on). Where the total cannot be applied (more than one circuit carries the type, or the total is not above the metered count) the screen says so.
...
- **Acceptance criteria:**
  ...
  - AC-5 (edge): Given a circuit fails a hard criterion (fixtures >15ft, on a driveway, no WiFi reach), when PER-04 flags it, then that circuit is marked ineligible and cannot be commissioned as it stands — the screen names WHICH criterion decided it, so the operator can act on it rather than only read a verdict. **Amended 2026-09-08 (the user's call, CON-16 amendment): an ineligible circuit is no longer a dead end** — see AC-11/AC-12.
- AC-11 (edge, added 2026-09-08): Given a candidate fails any CON-16 criterion — hard or the light-count minimum — when PER-01 (the `manage_survey` + `manage_pipeline` proxy) approves an exception with a stated reason, then the circuit becomes eligible, the criteria waived are recorded on it alongside who approved and why, and **the surveyor's own checklist answers are left exactly as recorded**. A non-PER-01 viewer is told who can do it. Waiving WiFi reach states on screen that FirsThing supplies its own 4G router, since the waiver commits the crew to bringing one.
- AC-12 (edge, added 2026-09-08): Given an answer was recorded wrongly on site, when PER-01 corrects the checklist with a stated basis, then eligibility is re-derived from the corrected answers and nothing is waived. Correcting and approving an exception are deliberately separate acts: one says the answer was wrong, the other says the answer stands and the circuit proceeds anyway. Neither is offered once the circuit has passed its eligibility decision — the checklist is what the commissioning work was authorised against.
...
- **Behavioral rules:** The computed percentage is written verbatim, never rounded — CON-20 is explicit that rounding to 60% is wrong, and AC-8 keeps that true by making any adjustment an explicit override instead. Writing `benchmarkSavingsPct` is the terminal, contract-defining action of this whole capability — once written, only a verified light-count-change event (CON-10) can trigger a rescale; it is never renegotiated.
- **Acceptance criteria:**
  ...
  - AC-3 (failure/anomaly): Identical restart mechanism to FEAT-012 — an anomaly resets the valid-day count from the following midnight. **The detection rule, however, is not identical (scope change, 2026-08-15):** a post-install day is anomalous when its own implied savings against the in-force baseline falls outside CON-20's 60-80% band, not when it varies more than ±5% from the running average. The ±5% rule is a self-consistency check, which is the only question available pre-install (there is no baseline yet); post-install there *is* a baseline, and CON-20 already says what a plausible day looks like against it. Running ±5% here flagged ordinary jitter between perfectly good days, and because an anomaly resets the count, a healthy circuit could never finish its window. The band also subsumes every data-quality case the ±5% rule caught: a dead meter reads as 100% savings, a 10× transcription error as negative savings — both outside the band by a wide margin. FEAT-045's ±5% is unchanged; it answers a different question (is this *day's reading* believable) on the billing-ingest path.
  - AC-6 (edge, added 2026-08-15): Given a circuit is genuinely under-performing, every post-install day flags under AC-3 and the window can never complete — so "record the fix and restart" is the wrong action, because nothing was fixed. PER-04 can instead escalate the recorded out-of-band days directly into FEAT-015's queue, carrying the readings that produced the verdict. Without this, FEAT-015's queue would be unreachable in practice: five days each judged in-band can only ever average in-band, so AC-5's completion path alone cannot raise a review.
  - AC-4 (permission): Given a non-PER-04/PER-01 actor, no visibility into triggering restarts. Benchmark-writing remains a system computation — but see AC-8, which adds a recorded override rather than leaving the figure unreachable.
  - AC-7 (scope change, 2026-08-27): **A circuit may be demonstrated more than once, and the benchmark is derived from the demos that count.** A demo can be rejected — the first ran badly and a second was done — in which case the surviving demo alone decides the benchmark. Or a second demo is run at the society's own request because they are not confident in the first, in which case **both are included and the benchmark is the simple average of their savings percentages**. This is existing practice, not new: Aditya Urban Casa's basement was demonstrated twice (100 lights at 48.28%, 22 lights at 85.19%) and its signed agreement carries 66.72%, which is exactly their mean. Each demo keeps its own metered light count, baseline, post-install average and daily readings, so two demos measured on overlapping dates no longer collide.
  - AC-8 (scope change, 2026-08-27): **The derived percentage can be overridden with a stated reason, and is otherwise stored exactly as measured.** There is deliberately no automatic rounding (the user's call): a stored benchmark is either what the demos measured or what someone deliberately chose, and rounding would make it a third thing that is neither. Rounding 64.16% to 64% is itself an override, recorded like any other. The override exists because the agreed benchmark is what a society is billed against and it does not always equal what the readings give — Aditya Mega City's tables give 58.48% raw or 64.16% with its excluded street lights, against the 64% its agreement states. An override records the figure, the reason, who set it and when, and the computed value stays visible beside it. **An override never moves a result into CON-20's 60-80% band**: the band is judged on what the demos measured, so overriding an out-of-band circuit is a deliberate, recorded act rather than a way to make FEAT-015's review disappear.
...
- **Acceptance criteria:**
  ...
  - AC-6 (happy, added 2026-08-15): Given a circuit has recorded out-of-band days under FEAT-014-AC-3 that a restart cannot resolve, when PER-04 escalates them, then a review is raised carrying those readings and the baseline they were judged against. Raising is PER-04's (they are on site recording days); resolving remains PER-01's under AC-4.
- **Release note (2026-08-15):** Scored R1, **built in R0** — the state it resolves is reachable in R0 the moment a post-install window runs, and without it a circuit that measures out of band parks permanently with nothing to act on. Pulled forward rather than left as a dead end, the same call recorded for FEAT-035-AC-6 at MS-06.
- **Permissions:** PER-01 (review, decide, escalate); PER-04 (raise, per AC-6).
...
- **Acceptance criteria:**
  ...
  - AC-7 (optional documents — amended 2026-09-15, the user's call): Given a society that is GST-registered but has not handed over the certificate (or has a tariff but not the bill), when operations records the GSTIN (or the unit rate in ₹/kWh) on the checklist, then the item is settled for GATE-01 and the deal moves on; the document stays wanted — the society's own page says "GST certificate not uploaded yet" / "Electricity bill not uploaded yet" with an Upload button that opens the documents tab with the type and society preselected, and the prompt clears once the document is filed and verified. A society that is not GST-registered is marked not applicable and never chased. The recorded tariff is where the offer's unit rate starts. KYC files are uploaded against the society, not a deal.
...
- **Acceptance criteria:**
  ...
  - AC-6 (worksheet — amended 2026-09-15, the user's redesign): Given the offer form, when the operator prices a deal, then the inputs are the figures the negotiation actually has — per light type: lights to install as per the agreement, the pre-installation consumption of those lights on a **chosen basis** — the demo circuit's measured baseline extrapolated per light, the theoretical draw of the agreed lights (watts × hours a day, from the load inventory and editable), or a custom figure typed per month (the daily figure is derived); a fresh sheet defaults to the higher of the first two ("we decide based on which is higher") and the basis is stored on the circuit term — and the agreed savings % (defaulting to the demo's figure and recorded as negotiated when changed); then the unit rate and the total expected saving in ₹ (bound both ways — editing either recomputes the other), the monthly amount payable to FirsThing, and spare lights as per the agreement. The society's take (saving − fee) and both parties' share percentages are derived and displayed, never typed; the stored `revenueSharePct` is the derived society share. The projected saving in kWh and ₹ is stored on the offer so it keeps saying what it was priced on (INV-02).
  - AC-7 (draft editable — amended 2026-09-15): Given a draft offer, when the operator changes any figure, then the draft is saved in place — it is a working document nobody outside has seen; issuing is what freezes it, after which a change is a counter (FEAT-028-AC-5).
  - AC-8 (population applied — amended 2026-09-15): Given an accepted offer whose agreed light count differs from a circuit's represented count, then acceptance applies the agreed count to the circuit as a `RepresentedCountChange` (a population correction, deliberately not an INV-07 rescale), so the month is never billed on a population the agreement does not state.
...
- **Detection threshold: ±5%, a platform constant, deliberately *not* the contracted tolerance band (decided 2026-08-14 at MS-07, user's explicit choice).** The range and day-over-day rules flag a day that moves more than 5% against the month's own reference. CON-01a's per-contract `tolerancePct` is a different quantity answering a different question — whether a *month's* consumption complies with the contracted benchmark — and wiring this detector to it would be a real defect in both directions: a ±10% contract would stop flagging genuinely implausible days, and every threshold change would become a commercial renegotiation. This is a data-quality rule about whether a *reading* is believable, and it carries no billing consequence of its own; the flags it raises are resolved by PER-01, and only an unresolved flag holds the month. Recorded because FEAT-045's own risk line left the number open pending real data — this is the first value, not the final one, and it lives in one named constant (`ANOMALY_TOLERANCE_PCT`) so tuning it stays a one-line change.
- **A reference built from non-zero days only.** The month's own median is the comparison point, taken across days that actually reported. A dead meter's run of zeros would otherwise drag the reference down until the working days started looking like the anomalies.
- **Acceptance criteria:**
...
- **Scope note (2026-09-22, CON-47):** this check runs only inside FEAT-048's own (now phase-two) automatic calculation, so it does not fire for an invoice-first month. What stands in for it today is narrower and reactive rather than this AC's proactive per-circuit band comparison: the release queue's own basis-regression check (built at MS-09, `release-triage.ts`) flags a circuit whose stats basis moved *measured → agreed* since its last released month, and a statistical check flags a total invoice far from the society's own trailing 3-month average. Neither reads the contract's own tolerance band or tracks a per-circuit consecutive-breach streak. FEAT-050 (the `pricingBasis` flip) and the whole CAP-05 deviation pipeline (FEAT-055–058) have nothing to act on without this check first raising a review, so they inherit the same deferral — all four are real, built, unit-tested code (`resolvePricingBasis` in `monthly-calculation.ts`; `DeviationReview` end to end through the `/admin/billing/deviations` queue) that simply isn't reachable from the primary monthly path until FEAT-048's dashboard-generated run is built for real.
...
- **Scope note (2026-09-22, CON-47):** the native `SavingsReport` row (narrative, provenance, deviation explanation) is written only by FEAT-048's own (now phase-two) calculation run, so no invoice-first month has one. What a society actually sees for those months is FEAT-111's published-months portal view (saved ₹, paid to FirsThing, kept, saved kWh, the weighted savings % and its basis in plain words) — built at MS-09, and traceable the same way (INV-02: every figure reads from the invoice line, the contract's term version, and the reading store where one exists). It satisfies this feature's *purpose* — a defensible monthly figure the society can read and trust — without being this feature's *artefact*: no per-figure provenance drill-down, no deviation narrative, no rendered/shareable document. The complete, generated-document version stays real, built, tested code, deferred alongside FEAT-048/049/050 to whenever dashboard-generated billing ships.
...
- **Open questions / assumptions:** none blocking.
- **Risks:** none significant.
- **Scope note (2026-09-22, CON-47):** delivery and release-gating are satisfied for the primary invoice-first path — `/portal/billing` and `/portal/electricity` show only RELEASED months (FEAT-054's gate), scoped server-side to the viewer's own society (INV-05, AC-4), with history retained across versions (a re-derived month keeps its predecessor visible, AC-5). What is not built is outbound delivery over the society's own channel (email/WhatsApp/etc.) — same as this feature's own "Complete version" line already named, and blocked on ADR-008 (email provider), still Proposed.
...
- **Acceptance criteria:**
  ...
  - AC-3 (failure/arithmetic): Given any line where Qty × Rate − Discount ≠ Amount, or the lines do not sum to the sub-total, tax or total, the row is flagged with both figures and cannot be submitted until corrected or acknowledged with a reason.
     **Amended 2026-09-16 (user's rule):** a difference of **₹0.50 or less** on a line, the sub-total or the total is Zoho's rounding to the rupee — it is accepted as reconciled, labelled "rounded off" with both figures stated, and neither warns nor asks for a reason. Only a difference beyond ₹0.50 flags. (Tax already allowed a rupee across the lines.)
  ...
  - AC-10 (edge/unmapped line): Given a service line that maps to no circuit of that society (a deal with no circuit on record), submit is blocked with the named gap and a link to create the circuit — a stat is never derived against no circuit.
   - AC-11 (edge/one line, several circuits — added 2026-09-16, user-specified): Given a service line whose count is the SUM of two or more circuits of one light type (the bill says "all lift-lobby lights" while the record holds a circuit per tower), the review lets the operator name each circuit and how many of the billed lights sit on it; a pair of same-type circuits whose counts add up exactly is proposed automatically; the split must add up to the line's quantity before submit; each circuit gets its own fee line with the line's amount shared in proportion to its lights (the parts adding back to the printed amount exactly), its own count disagreement and its own apply-forward; the invoice line points at no single circuit.
...
- **Acceptance criteria:**
  - AC-1 (happy/agreed — rewritten 2026-09-15, the user's rule): Given a submitted month whose circuit has no credible readings for it, when stats derive, then the `agreed` saving is what the fee is a share of under the agreement — saved ₹ = the line's amount ÷ FirsThing's share %, kept ₹ = saved − fee, saved kWh = saved ₹ ÷ the contract's unit rate — and the society is shown saved / paid to FirsThing / kept, each with provenance. Aditya Mega City July 2026: ₹14,050 ÷ 36% = ₹39,027.78 saved, ₹24,977.78 kept, asserted to the paisa in a unit test with the party named. A lump-sum deal (no share) falls back to (baseline ÷ metered) × billed × days × benchmark × rate.
  ...
  - AC-3 (failure/day validity — rewritten 2026-09-15, the user's rule): Given readings exist, each day is classified complete (all 24 hours with data, kWh > 0, saving ≤ 80%), partial, offline (a zero row) or suspect; only complete days feed the measured %, which is extrapolated to the month; the record states the split as a warning ("Only 4 of 31 days had complete readings — 7 partial, 20 with the meter offline — those days were not used"); fewer than 4 complete days → `agreed`, with the reason. The user's own example — 4 complete days at 60% among 7 partial and 20 offline — measures the month at 60%.
