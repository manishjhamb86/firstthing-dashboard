...
- **Acceptance criteria:**
  ...
  - AC-3 (failure/coverage or suspect): Given readings exist but cover fewer days than the floor, OR read as a saving above CON-45's 80% suspect bound (a dead meter), the basis is `agreed` and the reason — the coverage, or "96.8% — check the meter" — is stated on the record.
