...
- **Acceptance criteria:**
  ...
  - AC-7 (correction — amended 2026-09-16, the user's call: "on installation light count can change"): Given a recorded inventory area, when field staff correct its count (and method / note — an estimated count still needs its note), then the row is updated with old and new logged, the extrapolation-base totals re-read from it, and the screen states when the candidate circuit for that light type still represents the old figure — that count is corrected on the circuit's own page, where the offer and the bill read it; it is never moved silently.
