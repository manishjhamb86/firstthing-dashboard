...
| CMP-19 | **Grouped sidebar nav** (added 2026-09-15, user-asked: "too many menu tabs… few main sections and sub sections") | SUR-01 admin shell; the portal keeps its flat list (7 items) | Two levels only. Single items (Portfolio, Schedule) and **groups** named for the operator's domain, not the feature's age: Deals (pipeline · field work · documents) · Societies (societies · inspections · support) · Lighting (demo monitoring · live monitoring · meters · readings) · Water (tanks) · Billing (board · deviations · invoice intake) · Settings (device catalog · admin users). **One group open at a time** (user's call, 2026-09-15 — several open "defeats the whole purpose"): the group holding the current page is open on load; opening another folds it; clicking the open one folds everything; the choice is remembered per viewer while they stay on the page (`localStorage` — a convenience, never the record), and the current page's group wins again on a fresh load. A group whose every item the viewer lacks permission for is not rendered, so a field account sees Portfolio, Schedule, Deals and Societies and no empty headers. Counts stay on the item, never summed up to the group. Research: Shopify admin (domains with sub-items under the active one), Stripe (collapsible sections, one open), Atlassian/Linear (5–8 groups, home first, settings last) | collapsed / expanded / active-within | — |
