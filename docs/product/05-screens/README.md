...
| Open question | — | What counts as "identical" on reconciliation. Exact match would flag every row as a conflict when a vendor re-exports at a different decimal precision. Provisionally: equal when rounded to the stored precision | flagged on SCR-080 |
| Open question | — | Overwriting a month closed on SCR-082 but not yet released through SCR-092 currently invalidates the accountant's review silently | flagged on SCR-080 |
