...
export default async function TankStatusPage({
  ...
  return (
    <>
        ...
        action={
          tank.society && tank.hasLevelSignal ? (
            <ReassignButton
              tankName={tank.name}
              tankId={tank.id}
              currentSocietyId={tank.society.id}
              currentSetup={tank.setupType ?? null}
      ...
      {/* Two layouts (user's call, 2026-09-15). UNASSIGNED: the form and the
          level side by side, first — assigning is what the reader came to do.
          ASSIGNED: the form leaves the page (Reassign, top right, opens it in
          a modal), the history comes first, and the level sits beneath it
      ...
      <div className="mb-5 grid items-stretch gap-5 lg:grid-cols-12">
          <Card className="flex h-full flex-col p-5 sm:p-6 lg:col-span-7">
            ...
            {tank.hasLevelSignal ? (
              <>
                <span className="lbl mb-2">Assign to a society</span>
                <AssignControl
                  tankId={tank.id}
