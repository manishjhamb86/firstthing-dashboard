...
import { PreInstallDoc } from "@/app/admin/societies/[id]/circuits/[circuitId]/reports/pre-install-doc";

export const dynamic = "force-dynamic";

/**
 * The society's copy of a demo's Pre-installation consumption report.toLowerCase() (2026-09-27): the
...
export async function generateMetadata({ params }: { params: Promise<{ demoId: string }> }) {
  ...
  return { title: reportTitle("Pre-installation consumption report", d?.circuit.society.name, null) };
}

export default async function PortalPreInstallDocPage({ params }: { params: Promise<{ demoId: string }> }) {
  const viewer = await resolvePortalViewer();
  ...
  return <PreInstallDoc report={report} backHref="/portal/documents" />;
}
